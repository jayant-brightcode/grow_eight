...
export const OtpVerify = async(req,res)=>{
  ...
  try {
   ...
   if(find_user){
      ...
      await userModel.findByIdAndUpdate({_id:find_user._id},{notification_token:device_token})
  ...
  try {
      ...
      if(token.user_type == 2){
        ...
        if(searchquery){
           ...
           if(searchquery==""){
            const university = await universityModel.find({status:true})
            return res.status(200).json({
              university:university
            })
           }

          const university = await universityModel.find({$and:[{name:{$regex:userpattern,$options:'i'}},{status:true}]})
          return res.status(200).json({
            university:university
          })
          ...
          if(!category || !region || !course || !country ||!fee){
            ...
            console.log(fee)
             ...
             if(fee=="lessthan10lakh"){
              ...
              const university = await universityModel.find({$and:[{category:category},{status:true},{region:region},{country:country},{ total_fee: { $gte: minFee, $lte: maxFee } },{
                course: { $in: course }
              }]})
  ...
  try {
      ...
      if(token.user_type == 2){
       ...
       if(!id){
          ...
          const university = await universityModel.findOne({$and:[{_id:id},{status:true}]}).populate('course').populate('faculty').populate('facility').populate('required_document').populate('language_require').populate("galary").populate("category").populate("country")
...
export const ApplyToUniversity = async(req,res)=>{
  ...
  //  const course = await courseModel.findById({_id:course_id})
    ...
    const notify = new notificationModel({
         title:"Thank you for applying",
         body:"you have applied to "+university.name +" .we will update you for further process.",
         notification_type:5,
...
export const GetMyAppliedApplication = async(req,res)=>{
  ...
  try {
      ...
      if(token.user_type == 2){
          ...
          const apply = await applicationModel.find({user:req.userinfo._id}).populate('university').populate({
            path: 'university',
            populate: {
              path: 'country',
              model: 'country'
  ...
  try {
      ...
      if(token.user_type == 2){
         ...
         if(university_one && university_two && university_three=="" &&university_four==""){
          ...
          const find_one = await universityModel.findOne({$and:[{_id:university_one},{status:true}]}).populate('country')
          const find_two = await universityModel.findOne({$and:[{_id:university_two},{status:true}]}).populate('country')
         ...
         if(university_one && university_two && university_three &&university_four==""){
          ...
          const find_one = await universityModel.findOne({$and:[{_id:university_one},{status:true}]}).populate('country')
          const find_two = await universityModel.findOne({$and:[{_id:university_two},{status:true}]}).populate('country')
          const find_three = await universityModel.findOne({$and:[{_id:university_three},{status:true}]}).populate('country')
         ...
         if(university_one && university_two && university_three &&university_four){
          ...
          const find_two = await universityModel.findOne({$and:[{_id:university_two},{status:true}]}).populate('country')
          const find_three = await universityModel.findOne({$and:[{_id:university_three},{status:true}]}).populate('country')
          const find_four = await universityModel.findOne({$and:[{_id:university_four},{status:true}]}).populate('country')
...
export const getCategoryById = async(req,res)=>{
  try {
      ...
      if(token.admin_type == 1){
        ...
        const find_cat = await categoryModel.findOne({$and:[{_id:cat_id},{status:true}]})
...
export const GetUniversityByCategory = async(req,res)=>{
  ...
  try {
     ...
     if(token.user_type == 2){
       ...
       const s =  await universityModel.find({$and:[{ category: { $in: [c_id] } },{status:true}]});
        return res.status(200).json({
          university:s
        })
...
export const GetUniversityByCountry = async(req,res)=>{
  ...
  try {
     ...
     if(token.user_type == 2){
       ...
       const s =  await universityModel.find({$and:[{ country: c_id},{status:true}]});
        return res.status(200).json({
          university:s
        })
...
export const PopularCountry = async(req,res)=>{
  ...
  try {
     ...
     if(token.user_type == 2){
       ...
       const s =  await countryModel.find({status:true});
        return res.status(200).json({
          country:s
        })
...
export const GetCourseByCatId = async(req,res)=>{
  ...
  try {
     ...
     if(token.user_type == 2){
      ...
      const course  = await courseModel.find({$and:[{cat_id:cat_id},{status:true}]})
...
export const GetRegion = async(req,res)=>{
  ...
  try {
     ...
     if(token.user_type == 2){
      ...
      const region  = await regionModel.find({status:true})
  ...
  try {
     ...
     if(token.user_type == 2){
      ...
      if(!region){
       ...
       const s =  await countryModel.find({$and:[{region:region},{status:true}]});
        return res.status(200).json({
          country:s
        })
